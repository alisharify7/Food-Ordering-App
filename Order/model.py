...
Food2ReserveDay = sa.Table(
    BaseModel.set_table_name("food_2_day_reserve"),
    BaseModel.metadata,
    sa.Column(
        "food_menu_id",
        sa.INTEGER,
        sa.ForeignKey(BaseModel.set_table_name("food_menu") + ".id"),
        nullable=False,
    ),
    sa.Column(
        "food_reserve_day_id",
        sa.INTEGER,
        sa.ForeignKey(BaseModel.set_table_name("food_reserve_day") + ".id"),
        nullable=False,
    ),
)
...
class FoodReserveDay(BaseModel):
    __tablename__ = BaseModel.set_table_name("food_reserve_day")
...
class FoodType(BaseModel):
    __tablename__ = BaseModel.set_table_name("food_menu_type")
...
class Food(BaseModel):
    __tablename__ = BaseModel.set_table_name("food_menu")
    ...
    images: so.Mapped[sa.JSON] = so.mapped_column(
        sa.JSON, default=json.dumps([]), nullable=False, unique=False
    )
    type_id: so.Mapped[int] = so.mapped_column(
        sa.INTEGER,
        sa.ForeignKey(FoodType.id, ondelete="SET NULL"),
        unique=False,
        nullable=True,
    )
    reserve_days = so.relationship(
        FoodReserveDay, secondary=Food2ReserveDay, backref="foods_list", lazy="dynamic"
    )
...
class FoodPrice(BaseModel):
    __tablename__ = BaseModel.set_table_name("menu_prices")
    price: so.Mapped[int] = so.mapped_column(sa.BIGINT, unique=False, nullable=False)
    food_id: so.Mapped[int] = so.mapped_column(
        sa.INTEGER, sa.ForeignKey(Food.id), nullable=False
    )
...
Order2FoodPrice = sa.Table(
    BaseModel.set_table_name("order-2-food-price"),
    BaseModel.metadata,
    sa.Column("food_price_id", sa.INTEGER, sa.ForeignKey(FoodPrice.id), nullable=False),
    sa.Column(
        "order_id",
        sa.INTEGER,
        sa.ForeignKey(BaseModel.set_table_name("orders") + ".id"),
        nullable=False,
    ),
)
...
class Order(BaseModel):
    __tablename__ = BaseModel.set_table_name("orders")
    user_id: so.Mapped[int] = so.mapped_column(
        sa.INTEGER, sa.ForeignKey(User.id, ondelete="SET NULL"), nullable=True
    )
    prices = so.relationship(
        FoodPrice, secondary=Order2FoodPrice, backref="orders", lazy="dynamic"
    )
